...
state, KMP adapter contracts, iOS symmetry, async pipeline in Rust) have been
removed — see `IMPROVEMENTS.md` and the git history. The existing layering
...
## 3. Testing strategy (design-level)
...
## 4. Remaining Rust tooling & crates

| Tool / crate | Purpose | Notes |
| --- | --- | --- |
| `serde` + `serde_json` | persist `BenchmarkRun` | makes benchmark runs comparable/reproducible (§1) |
| `proptest` | property tests for filters | EWMA stays within input bounds, blend is a convex combination (§3) |
...
1. Property + golden-frame tests (§3), crate/tooling setup (§4).
...
(`BenchmarkRun` + model manifest) and the quality bar (property/golden-frame
tests, remaining crates).
