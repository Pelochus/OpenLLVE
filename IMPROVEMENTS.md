...
| P3.7 | `Pipeline` + `TemporalMode` rename (§12) | med | deferred to P1.2 | "Strategy" conflates model family and temporal mode; rename to `Pipeline` + `TemporalMode { Stateless, Recurrent }`. Not folded into P3.4/P3.5: the `TemporalMode` semantics are not final until the model lands (P1.2), and the rename touches core, FFI, docs, and Kotlin — do it once, with P1.2. |
