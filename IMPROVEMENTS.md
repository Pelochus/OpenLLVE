...
| P1.2 integrate a real model | ✅ done — `ModelRunner` in `core/src/model.rs` (feature `model`, ADR-0001) loads `zero-dce-int8.tflite` via `tflite-c-rs`; `LliePipeline::with_model` runs it (RGB `[0,1]` in → enhanced RGB `[0,1]` out, 4th brightness channel = global mean clamped to 0.5, tiling for >256×256, 8-curve application); FFI `openllve_pipeline_new_llie_with_model`; model path benchmarked; `LiteRTInferenceEngine.kt` deleted. The committed model is a re-derived fixed-256×256 variant (upstream 1×1×1 file kept as `zero-dce-int8-upstream.tflite`) because the TFLite C runtime segfaults on `ResizeInputTensor` for the original. Verified bit-exact vs upstream+resize under LiteRT, and the Rust pipeline matches the upstream Python reference (max diff ~0.008, XNNPACK-vs-reference kernel) |
| P1.3 KMP shared layer | ⬜ remaining — see §5 |
| P1.4 blend semantics | ✅ done — `llie.rs`/`temporal.rs` now blend the *current* raw frame (matching the documented intent); dead `previous_frame`/`state` fields removed; `LliePipeline::default()` no longer `unwrap`s; state-leak tests added |
...
| P3.4+P3.5 `Frame` type + out-buffer API | ✅ done — validated `Frame`/`FrameRef` views (`width`, `height`, `channels`, `stride` in bytes, `FrameFormat::F32`) + owned `OwnedFrame` for temporal state; `process(input: &FrameRef, output: &mut Frame)` everywhere (no per-frame `Vec` churn; blend has an in-place variant); FFI `openllve_process_frame` with independent in/out dims, checked dimension errors, ABI v2; `bytemuck` `from_bytes` typed views; bench/integration tests use pre-allocated double buffers |
| P3.3 crate additions | ⚠️ partial — `bytemuck` added with P3.4/P3.5; `serde`/`serde_json` wait for P3.6, `proptest` for P3.8 |
...
| Layer | Status |
| --- | --- |
| `core/` — Rust crate + C FFI (pipelines, filters, metrics) | Real code; **compiles and passes CI** (P0.1 done — see §0; §2.1 documents the original failure) |
...
4. **Pipeline model is an identity stub** — `LliePipeline::process` and `LlveTemporalPipeline::process` just copy the input; no model is invoked. Consequently `benches/inference_bench.rs` measures a memcpy, not inference, and the benchmark harness (warm-up exclusion, thermal tracking) is not wired to any real workload.

## 4. Concrete code-level bugs & smells

| Location | Issue |
| --- | --- |
| `core/src/pipelines/llie.rs` `process()` | Blend uses `self.previous_frame` (the **previous** raw frame) as the "raw" input, but the doc comment says "Blends the **raw input frame** with the AI-enhanced frame". Same pattern in `temporal.rs` (`self.state`). Either the semantics or the docs are wrong. |
| `core/src/pipelines/llie.rs` `Default` | `Self::new().unwrap()` in a `Default` impl — panics on an infallible path; fine today, but a footgun. |
...
| `core/src/ffi.rs` | FFI returns only `bool` — no error codes, no ABI version function, no documented thread-safety (stateful pipelines must not be shared across threads). C header uses opaque typedefs (`OpenLlvePipeline`) whose names don't match the Rust type (`OpenLlvePipelineEnum`) — harmless but confusing. `openllve_pipeline_process` silently accepts `output_len > input_len` and copies `min(...)`. |
...
| P3.7 | `Pipeline` + `TemporalMode` rename (§12) | med | ✅ done (with P1.2) | "Strategy" conflated model family and temporal mode; renamed to `Pipeline` + `TemporalMode { Stateless, Recurrent }`. `InferenceStrategy` → `Pipeline`, `LlieStrategy` → `LliePipeline`, `LlveTemporalStrategy` → `LlveTemporalPipeline`; `core/src/strategies/` → `core/src/pipelines/`; FFI `OpenLlveStrategy` → `OpenLlvePipeline` + `openllve_pipeline_*` names; C header updated; ABI bumped 2 → 3; Kotlin + docs updated. |
